...
class TextConverter():
	...
	def __init__(self, rel_path):
		self.TMP = os.path.join(rel_path, 'tmp/')
	...
	def convert(self, text):
		...
		with open(os.path.join(self.TMP,"dict"), "w") as f:
			for i in sorted(v):
	...
	def mlf(self, text, name):
		with open(os.path.join(self.TMP,"mlf.mlf"), "w") as f:
	...
	def scp(self, name):
		with open(os.path.join(self.TMP,"audios.scp"), "w") as f:
			f.write("%s %s\n" %(name, name.replace(".wav", ".mfc")))
		with open(os.path.join(self.TMP,"mfc.matl"), "w") as f:
			f.write("%s\n" %name.replace(".wav", ".mfc"))

	def generate_textgrid(self, name, repl="", mul_text=False):
		l = repl.split()
		try:
			with open(os.path.join(self.TMP,"recog.out"), "r") as w:
				for i in w:
					if(i[0]=='"'):
						with open("%s" %i[1:-2].replace(".rec", ".lab"), "w") as f:
							...
							f.write('\n'.join(new_file))
			if(mul_text):
				self.multi_split(name.replace(".wav", ".lab"), name.replace(".wav", ".TextGrid"), os.path.join(self.TMP,"text1.out"), os.path.join(self.TMP,"text2.out"))
			else:
	...
	def perform_conversion(text, filename, in_format, out_format, rel_path, aligner='HTK'):
		...
		if(in_format == 'graf'):
			text = converter.convert_sentence(text1, rel_path)
		...
		if(out_format == 'fonema'):
			...
			with open(os.path.join(self.TMP,"text1.out"), "w") as f:
				f.write(parser.parse(lexer.tokenize(" ".join(v))))
			# Conversao em palavras eh salva em text2.out
			with open(os.path.join(self.TMP,"text2.out"), "w") as tt:
				tt.write(text)
		...
		txt_cnv = TextConverter(rel_path)
		...
		if(aligner == 'HTK'):
			...
			with open(os.path.join(self.TMP,"train.lab"), "w") as f:
				f.write(text)
		return text

	@staticmethod
	def align(text, filename, in_format, out_format, base_dir, TMP, aligner='HTK', hmmdefs=False, config=False):
		print("aligner:", aligner)
		print("hmmdefs:", hmmdefs)
	...
	def format_output(text, filename, out_format, aligner='HTK'):
		...
		if(aligner == 'HTK'):
			...
			if('win' not in sys.platform.lower()):
				tc.textgrid_to_lab("output/corpus/train.TextGrid", os.path.join(self.TMP,"train.lab"))
			else:
				tc.textgrid_to_lab("output\\corpus\\train.TextGrid", os.path.join(self.TMP,"train.lab"))
			if(out_format=='todos'):
				tc.multi_split(os.path.join(self.TMP,"train.lab"), filename.replace(".wav",".TextGrid"), os.path.join(self.TMP,'text1.out'), os.path.join(self.TMP,'text2.out'))
			else:
				tc.lab_to_textgrid(os.path.join(self.TMP,"train.lab"), filename.replace(".wav", ".TextGrid"))
